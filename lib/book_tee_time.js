...
_numFreeSlots = function (slots) {
  ...
  for (var i = 0; i < slots.length; i++) if (slots[i]) numFree++
...
_get_tee_time = function (response, time) {
  ...
  if (index >= 0) {
    ...
    if (splitted[index + 4] == '<td></td>') freeSlots[0] = true

    if (splitted[index + 5] == '<td></td>') freeSlots[1] = true

    if (splitted[index + 6] == '<td></td>') freeSlots[2] = true

    if (splitted[index + 7] == '<td></td>') freeSlots[3] = true
...
_book_the_tee_time_recursive = function (
  whatDate,
  time,
  dateComesAlive,
  phpsessid,
  ...
  return new Promise(function (resolve, reject) {
    return _get_tee_time_page(whatDate, phpsessid).then(response => {
...
exports.book_the_tee_time_recursive = function (
  whatDate,
  time,
  dateComesAlive,
  phpsessid,
...
exports.book_tee_time = function (
  login,
  password,
  whatDate,
  time,
  ...
  return new Promise(function (resolve, reject) {
    ...
    request(options)
      .then(response => {
        if (response.body.indexOf('MEMBER LOGIN') >= 0) {
          console.log('Home Page loaded....')
          const cookies = utils.get_cookies(response.headers['set-cookie'])
          return {
            token: utils.get_key_value('_csrf_token', response.body),
            phpsessid: utils.get_cookie('PHPSESSID', cookies)
          }
      ...
      .then(data => {
        console.log(
          `Ready to Loginx LOGIN_PAGE: ${LOGIN_PAGE} username: ${login} password: ${password} _csrf_token: ${data.token} phpsessid: ${data.phpsessid}`
        )
        const options = {
          method: 'POST',
          uri: LOGIN_PAGE,
      ...
      .then(phpsessid => {
        return _book_the_tee_time_recursive(
          whatDate,
          time,
          dateComesAlive,
          phpsessid,
...
exports.login = async function (login, password) {
  ...
  if (response.body.indexOf('login_form_username') >= 0) {
    ...
    data = {
      token: utils.get_key_value('login_form[_token]', response.body),
      //phpsessid: utils.get_cookie('PHPSESSID', cookies)
      phpsessid: utils.get_cookie('rmm_sid', cookies)
    }
  } else throw new Error('home-page page not found')
  ...
  options = {
    ...
    headers: {
      'Content-Type': 'application/x-www-form-urlencoded',
      'User-Agent': 'R2D2 UA',
      Cookie: `rmm_sid=${data.phpsessid};consented=1`
    },
    transform: _include_headers,
    simple: false
  }
  ...
  if (response.body.indexOf('Redirecting to /wicklow') >= 0) {
    ...
    console.log(
      `Logging in username: LOGIN_PAGE: ${LOGIN_PAGE} uername: ${login} password: ${password} _token: ${data.token} rmm_sid: ${data.phpsessid} auth: ${data.auth}`
    )
    ...
    let options = {
      ...
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
        'User-Agent': 'R2D2 UA',
        Cookie: `rmm_sid=${data.phpsessid};consented=1;auth=${data.auth}`
      },
      transform: _include_headers,
      simple: false
    }
    ...
    return { status: status.LOGGED_IN, phpsessid: utils.get_cookie('PHPSESSID', cookies) }
  } else {
    throw new Error('Failed to login.....')
  }
}
